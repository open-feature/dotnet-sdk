...
public class EvaluationStepDefinitions
{
    ...
    public EvaluationStepDefinitions(State state)
    {
        this._state = state;
    }
    ...
    {
        this._state.Flag = new FlagState(flagKey, defaultValue.ToString(), FlagType.Boolean);
        this._state.FlagResult = await this._state.Client!.GetBooleanValueAsync(flagKey, defaultValue).ConfigureAwait(false);
    }

    [Then(@"the resolved boolean value should be ""(.*)""")]
    public void Thentheresolvedbooleanvalueshouldbe(bool expectedValue)
    {
        var result = this._state.FlagResult as bool?;
    ...
    {
        this._state.Flag = new FlagState(flagKey, defaultValue, FlagType.String);
        this._state.FlagResult = await this._state.Client!.GetStringValueAsync(flagKey, defaultValue).ConfigureAwait(false);
    }

    [Then(@"the resolved string value should be ""(.*)""")]
    public void Thentheresolvedstringvalueshouldbe(string expected)
    {
        var result = this._state.FlagResult as string;
    ...
    {
        this._state.Flag = new FlagState(flagKey, defaultValue.ToString(), FlagType.Integer);
        this._state.FlagResult = await this._state.Client!.GetIntegerValueAsync(flagKey, defaultValue).ConfigureAwait(false);
    }

    [Then(@"the resolved integer value should be (.*)")]
    public void Thentheresolvedintegervalueshouldbe(int expected)
    {
        var result = this._state.FlagResult as int?;
        Assert.Equal(expected, result);
    }
    ...
    {
        this._state.Flag = new FlagState(flagKey, defaultValue.ToString(), FlagType.Float);
        this._state.FlagResult = await this._state.Client!.GetDoubleValueAsync(flagKey, defaultValue).ConfigureAwait(false);
    }

    [Then(@"the resolved float value should be (.*)")]
    public void Thentheresolvedfloatvalueshouldbe(double expected)
    {
        var result = this._state.FlagResult as double?;
        Assert.Equal(expected, result);
    }
    ...
    {
        this._state.Flag = new FlagState(flagKey, null!, FlagType.Object);
        this._state.FlagResult = await this._state.Client!.GetObjectValueAsync(flagKey, new Value()).ConfigureAwait(false);
    }
    ...
    {
        Value? value = this._state.FlagResult as Value;
    ...
    {
        this._state.Flag = new FlagState(flagKey, defaultValue.ToString(), FlagType.Boolean);
        this._state.FlagEvaluationDetailsResult = await this._state.Client!.GetBooleanDetailsAsync(flagKey, defaultValue).ConfigureAwait(false);
    }
    ...
    {
        var result = this._state.FlagEvaluationDetailsResult as FlagEvaluationDetails<bool>;
    ...
    {
        this._state.Flag = new FlagState(flagKey, defaultValue, FlagType.String);
        this._state.FlagEvaluationDetailsResult = await this._state.Client!.GetStringDetailsAsync(flagKey, defaultValue).ConfigureAwait(false);
    }
    ...
    {
        var result = this._state.FlagEvaluationDetailsResult as FlagEvaluationDetails<string>;
        Assert.Equal(expectedValue, result?.Value);
        Assert.Equal(expectedVariant, result?.Variant);
        Assert.Equal(expectedReason, result?.Reason);
    }
    ...
    {
        this._state.Flag = new FlagState(flagKey, defaultValue.ToString(), FlagType.Integer);
        this._state.FlagEvaluationDetailsResult = await this._state.Client!.GetIntegerDetailsAsync(flagKey, defaultValue).ConfigureAwait(false);
    }
    ...
    {
        var result = this._state.FlagEvaluationDetailsResult as FlagEvaluationDetails<int>;
        Assert.Equal(expectedValue, result?.Value);
        Assert.Equal(expectedVariant, result?.Variant);
        Assert.Equal(expectedReason, result?.Reason);
    }
    ...
    {
        this._state.Flag = new FlagState(flagKey, defaultValue.ToString(), FlagType.Float);
        this._state.FlagEvaluationDetailsResult = await this._state.Client!.GetDoubleDetailsAsync(flagKey, defaultValue).ConfigureAwait(false);
    }
    ...
    {
        var result = this._state.FlagEvaluationDetailsResult as FlagEvaluationDetails<double>;
        Assert.Equal(expectedValue, result?.Value);
        Assert.Equal(expectedVariant, result?.Variant);
        Assert.Equal(expectedReason, result?.Reason);
    }
    ...
    {
        this._state.Flag = new FlagState(flagKey, null!, FlagType.Object);
        this._state.FlagEvaluationDetailsResult = await this._state.Client!.GetObjectDetailsAsync(flagKey, new Value()).ConfigureAwait(false);
    }
    ...
    {
        var result = this._state.FlagEvaluationDetailsResult as FlagEvaluationDetails<Value>;
    ...
    {
        this._state.Flag = new FlagState(flagKey, defaultValue, FlagType.String);
        this._state.FlagResult = await this._state.Client!.GetStringValueAsync(flagKey, defaultValue, this._state.EvaluationContext).ConfigureAwait(false);
    }
    ...
    {
        var key = this._state.Flag!.Key;
        var defaultValue = this._state.Flag.DefaultValue;

        string? emptyContextValue = await this._state.Client!.GetStringValueAsync(key, defaultValue, EvaluationContext.Empty).ConfigureAwait(false);
    ...
    {
        this._state.Flag = new FlagState(flagKey, defaultValue.ToString(), FlagType.String);
        this._state.FlagEvaluationDetailsResult = await this._state.Client!.GetIntegerDetailsAsync(flagKey, defaultValue).ConfigureAwait(false);
    }
    ...
    {
        var result = this._state.FlagEvaluationDetailsResult as FlagEvaluationDetails<int>;
        var defaultValue = int.Parse(this._state.Flag!.DefaultValue);
        Assert.Equal(defaultValue, result?.Value);
    }
